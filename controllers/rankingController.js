...
const getRanking = async (req, res) => {
  ...
  try {
    ...
    const leagueIds = [...new Set(bets.map(b => Number(b.leagueId || 1)).filter(Number.isFinite))];
    const rulesEntries = await Promise.all(
      leagueIds.map(async id => [id, await PointsService.getScoringRules(id)])
    );
    const rulesByLeague = new Map(rulesEntries);
    ...
    const unsortedRanking = bets.map(bet => {
      ...
      for (const gm of allUserMatches) {
        ...
        const scoringRules = rulesByLeague.get(Number(bet.leagueId || m.leagueId || 1)) || PointsService.DEFAULT_SCORING_RULES;
        const calculated = PointsService.calculateMatchPoints(gm, m, scoringRules);
        totalPoints += calculated.total;
        if (m.phase === 'group') groupPhasePoints += calculated.total;
        else knockoutPoints += calculated.total;
        continue;
