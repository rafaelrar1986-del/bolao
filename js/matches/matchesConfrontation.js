...
function getStageKey(match) {
  // A identidade da etapa deve ser a mesma usada pelo backend.
  // phaseName/roundName são preferidos; roundNumber só entra como fallback.
  return normalize(
    match?.phaseName ?? match?.roundName ?? match?.group ?? match?.roundNumber ?? ''
  );
}

function getTieKey(match) {
...
export function getKnockoutConfrontationInfo(match, allMatches = [], settings = {}) {
  ...
  if (!match || getEffectiveStageFormat(match, settings) !== 'home_away') {
  ...
  const legs = matches
    .filter(candidate => {
      ...
      const candidateTieKey = getTieKey(candidate);
      const currentTieKey = getTieKey(match);

      if (currentTieKey && candidateTieKey) {
        return candidateTieKey === currentTieKey;
      }
...
export function resolveKnockoutConfrontationQualifier(match, info, settings = {}) {
  ...
  const expectedLegs = getEffectiveStageFormat(match, settings) === 'home_away' ? 2 : 1;
  const legs = (confrontation.legs || []).slice(0, expectedLegs);

  if (!match || legs.length < expectedLegs || !legs.every(leg => leg.status === 'finished')) return null;

  if (expectedLegs === 1) {
    const only = legs[0];
    if (only.penaltiesA != null && only.penaltiesB != null &&
        Number(only.penaltiesA) !== Number(only.penaltiesB)) {
      return Number(only.penaltiesA) > Number(only.penaltiesB) ? 'A' : 'B';
    }
    if (only.scoreA != null && only.scoreB != null && Number(only.scoreA) !== Number(only.scoreB)) {
      return Number(only.scoreA) > Number(only.scoreB) ? 'A' : 'B';
    }
    return null;
  }
...
export function getKnockoutConfrontationPointContext({
  ...
  const rules = getRules(settings);
  if (!match || getEffectiveStageFormat(match, settings) !== 'home_away' || !isKnockoutMatch(match)) {
    return {
      match,
      betMatch,
