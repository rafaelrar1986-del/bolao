            ...
            <div class="user-row" style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:#222;border-radius:8px;margin-bottom:8px;border-left:4px solid ${user.hasPaid ? '#00ff00' : '#ffcc00'};">
                ...
                <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;justify-content:flex-end;">
                    ...
                    ${user.isAdmin
                      ...
                      : `<button class="btn btn-outline-primary btn-sm" onclick="makeUserAdmin('${safeId === 'self' ? '' : String(user.email || '').replace(/\\/g, '\\\\').replace(/'/g, "\\'")}', '${safeName}')">Tornar Admin</button>`
                    }
                    <button class="btn btn-outline-danger btn-sm" onclick="deleteAdminUser('${safeId}', '${safeName}')">Excluir</button>
                </div>
            </div>`;
        }).join('');
    } catch (err) {
...
registerAdminFunctions({openSetPodiumModal: openSetPodiumModal, resetAllBets: resetAllBets, openWhitelistModal: openWhitelistModal, loadWhitelist: loadWhitelist, removeWhitelistEmail: removeWhitelistEmail, makeUserAdmin: makeUserAdmin, deleteAdminUser: deleteAdminUser, loadStatsLockStatus: loadStatsLockStatus, updateStatsBtnUI: updateStatsBtnUI, toggleStatsLock: toggleStatsLock, openEmailModal: openEmailModal, loadAdminUsers: loadAdminUsers});
