...
router.post('/', protect, async (req, res) => {
  try {
    ...
    const lastMessage = await NewsMessage.findOne({
      user: req.user._id,
      leagueId: toLeagueId(leagueId), // 👈 Restringe o cooldown à liga atual
    ...
    const msg = await NewsMessage.create({
      user: req.user._id,
      leagueId: toLeagueId(leagueId), // 👈 Salva o vínculo com a liga
      text
    });
...
router.get('/', protect, async (req, res) => {
  try {
    ...
    const messages = await NewsMessage.find({ leagueId: toLeagueId(leagueId) }) // 👈 Filtro crucial
      .sort({ createdAt: -1 })
      .limit(20)
      .populate('user', 'name')
      .lean();
