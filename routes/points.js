...
router.post('/process-podium', protect, admin, async (req, res) => {
  try {
    ...
    // 🆕 CORREÇÃO: Converte objeto {first, second, third, fourth} para array [String]
    // O schema Settings.podium e o pointsService esperam array de strings
    const podiumArray = [first, second, third, fourth].filter(v => v != null && String(v).trim() !== '');
    const normalizedLeagueId = toLeagueId(leagueId);
    const result = await PointsService.setPodium(normalizedLeagueId, podiumArray);
    await rebuildLeagueDailyHistory(normalizedLeagueId);
...
router.get('/podium', protect, async (req, res) => {
  try {
    ...
    const podium = await PointsService.getPodium(toLeagueId(leagueId));
...
router.post('/podium/reset', protect, admin, async (req, res) => {
  try {
    ...
    const normalizedLeagueId = toLeagueId(leagueId);
    const result = await PointsService.resetPodium(normalizedLeagueId);
    await rebuildLeagueDailyHistory(normalizedLeagueId);
    return res.json({
      success: true,
...
router.post('/recalculate-all', protect, admin, async (req, res) => {
  try {
    ...
    const result = await PointsService.recalculateAllPoints(toLeagueId(leagueId));
    return res.json({
      success: true,
...
router.get('/integrity-check', protect, admin, async (req, res) => {
  try {
    ...
    const query = { leagueId: toLeagueId(leagueId) };
