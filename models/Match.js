...
MatchSchema.statics.markLeagueStarted = async function (leagueId, startedAt = new Date()) {
  ...
  const existing = await Settings.findById(normalizedLeagueId)
    .select('firstMatchStartedAt')
    .lean();
  ...
  if (!existing.firstMatchStartedAt) {
    return Settings.findOneAndUpdate(
      {
        _id: normalizedLeagueId,
        $or: [
          { firstMatchStartedAt: null },
          { firstMatchStartedAt: { $exists: false } }
        ]
      },
      {
        $set: {
...
MatchSchema.pre('save', async function (next) {
  try {
    ...
    const startedNow =
      liveStatuses.includes(this.status) &&
      (
        this.isNew ||
        this.isModified('status')
      );

    if (startedNow && this.leagueId) {
      await this.constructor.markLeagueStarted(
        this.leagueId,
        new Date()
      );
    }
...
async function protectImmutableMatchFields(query, next) {
  try {
    ...
    const current = await query.model.findOne(filter)
      .select('status phase date')
      .lean();

    if (current) {
      if (
        $set.phase !== undefined &&
        String($set.phase) !== String(current.phase)
      ) {
        delete $set.phase;
      }
      ...
      if (
        ...
        $set.date !== undefined &&
        String($set.date) !== String(current.date)
      ) {
        delete $set.date;
      }
    }
...
MatchSchema.pre('findOneAndUpdate', protectImmutableMatchFields);
MatchSchema.pre('updateOne', protectImmutableMatchFields);
...
MatchSchema.statics.finishMatch = async function (matchId, scoreA, scoreB, penA = null, penB = null, regA = null, regB = null, qSide = null) {
  ...
  const newPenA = penA !== null && penA !== undefined ? Number(penA) : null;
  const newPenB = penB !== null && penB !== undefined ? Number(penB) : null;

  const qSideWillChange =
    qSide !== null &&
    ['A', 'B'].includes(qSide) &&
    match.qualifiedSide !== qSide;

  // Early return só acontece quando TODOS os dados do resultado
  // permanecem iguais, incluindo pênaltis.
 ...
 */
MatchSchema.statics.unfinishMatch = async function (
  matchId,
  leagueId,
  statusBack = 'scheduled'
) {
  ...
  if (normalizedLeagueId) {
    filter.leagueId = normalizedLeagueId;
  }
  ...
  const alreadyReset =
    ...
    Array.isArray(match.goalsDetail) &&
    match.goalsDetail.length === 0 &&
    Array.isArray(match.statistics) &&
    match.statistics.length === 0 &&
    Array.isArray(match.shootoutDetail) &&
    match.shootoutDetail.length === 0;
